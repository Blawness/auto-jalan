...
test("Bottom navbar navigates between main pages", async ({ page }) => {
  ...
  await page.locator('a[href="/akun"]').last().click({ force: true })
  await expect(page).toHaveURL(/\/akun/)
  await page.locator('a[href="/lobby"]').last().click({ force: true })
  await expect(page).toHaveURL(/\/lobby/)
  await page.locator('a[href="/riwayat"]').last().click({ force: true })
  await expect(page).toHaveURL(/\/riwayat/)
  await page.locator('a[href="/keranjang"]').last().click({ force: true })
